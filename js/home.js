...
const app = class {
  ...
  setDefaultText(app, id) {
    let textOfLan = app.text[app.language == "Tiếng Việt" ? "vi" : "en"];
    let currentText = textOfLan.find((text, index) => {
      return text.id == id;
    });
    currentText = currentText || textOfLan[0];
    app.currentText = currentText.content;
    app.currentIndexWord = 0;
    app.renderText(app);
  }
  changeCurrentText(app) {
    let textOfLan = app.text[app.language == "Tiếng Việt" ? "vi" : "en"];
    let text = textOfLan[Math.floor(textOfLan.length * Math.random())];
  ...
  subRender(app) {
      ...
      render(instance) {
        ...
        popper.innerHTML = `
          <ul id="avt-headless">
            ...
            <li id="logout-btn">
              <i class="fa-solid fa-right-from-bracket fa-flip"></i>
            <span>Đăng xuất</span>
            </li>
          </ul> 
       `;
    ...
    headless[0].popper.onclick = function (e) {
