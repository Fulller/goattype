...
//Handle change language and update into local storage
function updateLangtoapp(lang) {
  let app = JSON.parse(localStorage.getItem("app"));
  app.language = lang;
  localStorage.setItem("app", JSON.stringify(app));
}

let langx = localStorage.getItem("lang") == "en" ? "English" : "Tiếng Việt";
updateLangtoapp(langx);
$$(".dropdown-item").forEach((btn) => {
  btn.onclick = function (e) {
    let lang =
      e.target.getAttribute("data-lang") == "en" ? "English" : "Tiếng Việt";
    updateLangtoapp(lang);
  };
});
